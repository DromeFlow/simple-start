...
import {
  countProfissionais,
  countRecrutadora,
  countProcessedDataForPeriod,
  getMonthlyActivitySummary,
  MonthlyActivitySummary,
  getProfessionalMonthlyStats,
  ProfessionalMonthlyStat,
  getProfessionalAppointmentsForPeriod,
  type ProfessionalAppointment,
  getRecrutadoraMonthlyMetrics,
  type RecrutadoraMonthlyMetrics,
  getProfissionaisActivatedForPeriod,
  getLastAppointmentByProfessional,
} from "../../services/analytics/prestadoras.service";
import {
  fetchProfissionais,
  updateProfissionalStatus,
  Profissional,
} from "../../services/profissionais/profissionais.service";
  ...
  >
    <div className="flex items-center gap-2">
      ...
      {showValue && value !== undefined && (
        <span className={`ml-auto text-lg font-bold ${isSelected ? "text-white" : "text-text-primary"}`}>{value}</span>
      )}
    </div>
  </button>
);
...
const PeriodDropdown: React.FC<{
  ...
  const years =
    availableYears && availableYears.length > 0 ? availableYears : [currentYear, currentYear - 1, currentYear - 2];
  const months = [
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center justify-between w-64 px-3 py-2 text-left border rounded-md bg-bg-secondary border-border-secondary"
      >
            ...
            <div className="py-1">
              {options.map((opt) => (
                <button
                  ...
                  className={`w-full px-3 py-2 text-left text-sm transition-colors hover:bg-bg-tertiary ${value === opt.value ? "bg-accent-primary text-white" : "text-text-primary"}`}
                >
                  {opt.label}
                </button>
              ))}
...
const PrestadorasPage: React.FC = () => {
  ...
  const [activePanel, setActivePanel] = useState<"profissionais" | "atuantes" | "recrutadora" | null>(null);
  ...
  const load = useCallback(async () => {
    if (!selectedUnit) {
      setProfissionaisCount(0);
      setRecrutadoraCount(0);
      setProcessedCount(0);
      return;
    }
    setLoading(true);
  ...
  const handleToggleStatus = async (profissional: Profissional) => {
    ...
    setProfissionaisList((prev) => prev.map((r) => (r.id === profissional.id ? { ...r, status: newStatus } : r)));
    ...
    try {
      ...
      setProfissionaisList((prev) =>
        prev.map((r) => (r.id === profissional.id ? { ...r, status: profissional.status } : r)),
      );
    } finally {
  ...
  const profMetrics = useMemo(() => {
    ...
    const ativas = profissionaisList.filter(
      (r) => normalizeStatus(r.status) === "ativa" || normalizeStatus(r.status) === "ativo",
    ).length;
    const inativas = profissionaisList.filter(
      (r) => normalizeStatus(r.status) === "inativa" || normalizeStatus(r.status) === "inativo",
    ).length;
  ...
  const filteredProfRows = useMemo(() => {
    ...
    return filteredByStatus.filter(
      (r) => (r.nome || "").toLowerCase().includes(q) || (r.whatsapp || "").toLowerCase().includes(q),
    );
            ...
            <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    <div className="p-4 bg-bg-tertiary rounded-lg text-center border border-transparent">
                      ...
                      <p className="text-2xl font-bold text-text-primary">
                        {busyCard === "profissionais" ? "..." : profissionaisCount}
                      </p>
                    </div>
                    <div className="p-4 bg-bg-tertiary rounded-lg text-center border border-transparent">
                    ...
                    <div className="p-4 bg-orange-100 dark:bg-orange-900/20 rounded-lg text-center border border-transparent">
                      ...
                      <p className="text-2xl font-bold text-orange-800 dark:text-orange-300">
                        {profissionaisAtivasSemAtendimento}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                    ...
                    <div className="p-4 bg-bg-tertiary rounded-lg text-center">
                      ...
                      <p className="text-2xl font-bold text-text-primary">
                        {profClickSummary.mediaAtendPorProfissional.toFixed(2)}
                      </p>
                    </div>
                    <div className="p-4 bg-bg-tertiary rounded-lg text-center">
                      <p className="text-sm text-text-secondary">Média por atendimento</p>
                      <p className="text-2xl font-bold text-text-primary">
                        {profClickSummary.mediaRepassePorAtendimento.toLocaleString("pt-BR", {
                          style: "currency",
                          currency: "BRL",
                        })}
                      </p>
                    ...
                    <div className="p-4 bg-bg-tertiary rounded-lg text-center">
                      ...
                      <p className="text-2xl font-bold text-text-primary">
                        {profClickSummary.profissionaisAtuantes > 0
                          ? (profClickSummary.totalRepasse / profClickSummary.profissionaisAtuantes).toLocaleString(
                              "pt-BR",
                ...
                <>
                        ...
                        {paginatedProfRows.length === 0 ? (
                          <tr>
                            <td className="px-4 py-6 text-center text-text-secondary" colSpan={5}>
                              {searchProfTerm
                                ? "Nenhuma profissional encontrada para a busca."
                          ...
                          paginatedProfRows.map((r, idx) => {
                            ...
                            if (ultimaData) {
                              ...
                              const diffDias = Math.floor(
                                (hoje.getTime() - dataUltimo.getTime()) / (1000 * 60 * 60 * 24),
                              );
                            ...
                            return (
                              ...
                              >
                                ...
                                <td
                                  className="px-4 py-2 text-text-secondary text-center truncate"
                                  title={r.whatsapp || "-"}
                                >
                                  {r.whatsapp || "-"}
                                </td>
                                <td className={`px-4 py-2 text-center ${ultimoClass}`}>{ultimoTexto}</td>
                                <td className="px-4 py-2 text-center" onClick={(e) => e.stopPropagation()}>
                                  <span
                                    className={`inline-flex items-center justify-center rounded-md text-xs font-semibold px-3 h-7 tracking-wide border focus:outline-none focus:ring-2 focus:ring-offset-1 transition shadow-sm ${
                                      isAtiva
                  ...
                  {profTotalPages > 1 && (
                    <div className="flex items-center justify-between p-4 border-t border-border-secondary bg-bg-secondary text-xs text-text-secondary">
                      <div>
                        Mostrando {profStart + 1} - {Math.min(profEnd, filteredProfRows.length)} de{" "}
                        {filteredProfRows.length}
                      </div>
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => setProfCurrentPage(1)}
                          disabled={profCurrentPage === 1}
                          className={`px-2 py-1 rounded-md border ${profCurrentPage === 1 ? "opacity-40 cursor-not-allowed" : "hover:bg-bg-tertiary"}`}
                        >
                          «
                        </button>
                        <button
                          onClick={() => setProfCurrentPage(profCurrentPage - 1)}
                          disabled={profCurrentPage === 1}
                          className={`px-2 py-1 rounded-md border ${profCurrentPage === 1 ? "opacity-40 cursor-not-allowed" : "hover:bg-bg-tertiary"}`}
                        >
                          ‹
                        ...
                        {Array.from({ length: Math.min(5, profTotalPages) }, (_, i) => {
                          ...
                          return (
                            <button
                              key={page}
                              onClick={() => setProfCurrentPage(page)}
                              className={`px-2 py-1 rounded-md border min-w-[32px] ${page === profCurrentPage ? "bg-accent-primary text-white border-accent-secondary" : "hover:bg-bg-tertiary"}`}
                            >
                              {page}
                            </button>
                          );
                        ...
                        <button
                          onClick={() => setProfCurrentPage(profCurrentPage + 1)}
                          disabled={profCurrentPage === profTotalPages}
                          className={`px-2 py-1 rounded-md border ${profCurrentPage === profTotalPages ? "opacity-40 cursor-not-allowed" : "hover:bg-bg-tertiary"}`}
                        >
                          ›
                        </button>
                        <button
                          onClick={() => setProfCurrentPage(profTotalPages)}
                          disabled={profCurrentPage === profTotalPages}
                          className={`px-2 py-1 rounded-md border ${profCurrentPage === profTotalPages ? "opacity-40 cursor-not-allowed" : "hover:bg-bg-tertiary"}`}
                        >
                          »
                ...
                <>
                  <div className="flex items-center justify-between border-t border-border-secondary p-6 pb-4">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <button
                        className={`px-3 py-1.5 rounded-md border transition-colors ${rankSort === "atendimentos" ? "bg-accent-primary text-white border-transparent" : "border-border-secondary text-text-primary hover:bg-bg-tertiary"}`}
                        onClick={() => setRankSort("atendimentos")}
                      >
                        Atendimentos
                      </button>
                      <button
                        className={`px-3 py-1.5 rounded-md border transition-colors ${rankSort === "ganhos" ? "bg-accent-primary text-white border-transparent" : "border-border-secondary text-text-primary hover:bg-bg-tertiary"}`}
                        onClick={() => setRankSort("ganhos")}
                      >
                        Ganhos
                      </button>
                    </div>
                  ...
                  <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
                    <table className="min-w-full text-sm">
                      <thead className="sticky top-0 z-10 bg-bg-tertiary shadow-sm">
                        <tr>
                          <th className="px-2 py-3 text-center text-xs font-medium text-text-secondary uppercase tracking-wider">
                            #
                          </th>
                      ...
                      <tbody>
                        {[...ranking]
                          .sort((a, b) =>
                            rankSort === "atendimentos"
                              ? b.atendimentos - a.atendimentos || b.totalRepasse - a.totalRepasse
                              : b.totalRepasse - a.totalRepasse || b.atendimentos - a.atendimentos,
                          )
                          .map((r, idx) => (
                            <tr
                              key={`${r.profissional}-${idx}`}
                              className="hover:bg-bg-tertiary cursor-pointer transition-colors duration-150 border-t border-border-secondary"
                              onClick={() => handleOpenAppointments(r.profissional)}
                            >
                              <td className="px-2 py-2 text-center text-text-secondary text-xs">{idx + 1}</td>
                              <td
                                className="px-4 py-2 text-sm font-medium text-text-primary truncate whitespace-nowrap"
                                title={r.profissional}
                              >
                                {r.profissional}
                              </td>
                              <td className="px-4 py-2 text-sm text-center text-text-primary">{r.atendimentos}</td>
                              <td className="px-4 py-2 text-sm text-center text-text-primary">
                                {r.totalRepasse.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                              </td>
                              <td className="px-4 py-2 text-sm text-center text-text-primary">
                                {(r.atendimentos ? r.totalRepasse / r.atendimentos : 0).toLocaleString("pt-BR", {
                                  style: "currency",
                                  currency: "BRL",
                                })}
                              </td>
              ...
              {!profClickSummary && (
                <div className="p-6 text-sm text-text-secondary">
                  <p>
                    Base para métricas: profissionais, recrutadora e processed_data. Clique no card "Profissionais
                    (ativos)" para ver médias iniciais do mês.
                  </p>
                </div>
              )}
            </>
